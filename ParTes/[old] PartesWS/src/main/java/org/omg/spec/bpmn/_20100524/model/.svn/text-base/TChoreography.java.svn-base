    ...
    extends TCollaboration
{
    ...
    public Vector<TFlowElement> getActionableFlowElements(){
		Vector<TFlowElement> actionableFlowElements = new Vector<TFlowElement>();
				
		Iterator<JAXBElement<? extends TFlowElement>> flowElementIterator = this.flowElement.iterator();
		while (flowElementIterator.hasNext()){
			TFlowElement fe = flowElementIterator.next().getValue();
			if(fe.isActionable()){
				actionableFlowElements.add(fe);
			}
		}
		return actionableFlowElements;
	}
