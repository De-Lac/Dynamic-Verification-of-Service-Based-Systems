...
public class CopyOfReduceParallelGateway {
	
	private AffectionsGraph originalAg;
	private HashSet <AffectionSet> tracesAffectionSets;
	private InterleavingTree it;
	private Log log = LogFactory.getLog(CopyOfReduceParallelGateway.class);

	
	public CopyOfReduceParallelGateway()
	{
	...
	public InterleavingTree reduceParallelGateway(ParallelExecution pg, WorkFlowGraph g) 
	{
		...
		create_graph(pg.getSuccessors(), g);
		explore_graph();
		printDebug("\n\n\n============== results ============");
...
//				 }
		 ...
		 if (interleaving == null)  
			 ...
			 for (MessageExchange ct : interleaving)
				 ...
				 if (! isValid(node))
			 		...
			 		fakeNode.setSender(null); fakeNode.setRequest(null); fakeNode.setRequest(null);
	 ...
	 */
	private  void create_graph(EList<WfgNode> directSucessorsOfParEx, final WorkFlowGraph g) 
	{
		printDebug("======= GRAPH CREATION ========");
		ArrayList<Thread> threads = new ArrayList<Thread>();
		printDebug( "creating structural transitions.");
		for (int i=0; i<directSucessorsOfParEx.size();i++) 
			{	
			 final MessageExchange ct = (MessageExchange) directSucessorsOfParEx.get(i);	
			 Thread t = new Thread ()
			 {	 
			  public void run()
			    {
				  printDebug("thread "+this.getId()+" started");
				  create_structuralTransitions(new GraphNode(ct), originalAg, g); 
 ...
 * @param branchNumber 
 */
private void create_structuralTransitions(GraphNode gn, AffectionsGraph ag, WorkFlowGraph g)
{
 ...
 if ( ! (gn.getCt().getSuccessor() instanceof Gateway))
	 ...
	 create_structuralTransitions(nextGn, ag,g);
 	}
 else
 	{ag.addEndingGN(gn);}
 g.removeNode(gn.getCt());
}
...
//
//		
//		Participant uno = WorkflowgraphFactory.eINSTANCE.createParticipant(); uno.setName("1");
//		Participant due = WorkflowgraphFactory.eINSTANCE.createParticipant(); due.setName("2");
//		Participant tre = WorkflowgraphFactory.eINSTANCE.createParticipant(); tre.setName("3");
//		Participant quattro = WorkflowgraphFactory.eINSTANCE.createParticipant(); quattro.setName("4");
//		Participant cinque = WorkflowgraphFactory.eINSTANCE.createParticipant(); cinque.setName("5");
//		Participant sei = WorkflowgraphFactory.eINSTANCE.createParticipant(); sei.setName("6");
//		
//		WorkFlowGraph wg = WorkflowgraphFactory.eINSTANCE.createWorkFlowGraph();
//		
//		{
//		 a = wg.createNewMessageExchange(); a.setName("A"); a.setId("A");
//		 a.setSender(tre); a.setReceiver(cinque);
//		}
//		{
//		 b = wg.createNewMessageExchange(); b.setName("B");b.setId("B");
//		 b.setSender(uno); b.setReceiver(due);
//		}
//		{
//		 c = wg.createNewMessageExchange(); c.setName("C");c.setId("C");
//		 c.setSender(uno); c.setReceiver(quattro);
//		}
//		{
//		 d = wg.createNewMessageExchange(); d.setName("D");d.setId("D");
//		 d.setSender(quattro); d.setReceiver(tre);
//		}
//		{
//		 e = wg.createNewMessageExchange(); e.setName("E");e.setId("E");
//		 e.setSender(due); e.setReceiver(tre);
//		}
//
//				
//		
//		ParallelExecution sag = wg.createNewParallelExecution("start");
//		ParallelExecution eag = wg.createNewParallelExecution("end");
//		sag.add_set_successor(a);/*wg.addControlFlow(uno, due);*/ a.add_set_successor(eag);
//		sag.add_set_successor(b); b.add_set_successor(eag);
//		sag.add_set_successor(c); c.add_set_successor(eag);
//		sag.add_set_successor(d); d.add_set_successor(eag);
//		sag.add_set_successor(e); e.add_set_successor(eag);
//		ReduceParallelGateway tis = new ReduceParallelGateway();
//		tis.create_graph(sag.getSuccessors(), wg);
//		tis.explore_graph();
//		tis.printDebug("\n\n\n============== results ============");
...
//	}
	...
	private CopyOfReduceParallelGateway getOuterType() {
		return CopyOfReduceParallelGateway.this;
	}
	}
